...
PLANES = {
    ...
    "Pro": {
        ...
        "descripcion": "Informe PDF premium y soporte prioritario",
    },
    "Business": {
        "precio_eur": 499,
...
def asegurar_columnas_premium():
    """Añade columnas necesarias para PDF premium (multi-tenant)."""
    conn = obtener_conexion()
    cursor = conn.cursor()
    try:
        cursor.execute(
        ...
        if cursor.rowcount == 0:
            ...
            cursor.execute(
                ...
                INSERT INTO clientes_purificados
                    (empresa, ID_Cliente, Nombre, Email, Edad, Ingresos_Anuales, Telefono, pdf_informe, plan_stripe)
...
def procesar_pago_exitoso(session_id):
    """Tras Checkout de suscripción: desbloquea plan y PDF premium si aplica."""
...
modo = st.sidebar.radio(
    "Módulos",
    ["Pipeline de Auditoría", "Base de Datos SQL"],
)
